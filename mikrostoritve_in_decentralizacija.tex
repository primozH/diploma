...
Veriženje podatkovnih blokov je peer-to-peer porazdeljena podatkovna shramba, dosežena s konsenzom, sistemom "pametnih" pogodb ter drugih pomožnih tehnologij \cite{hyperledgerWeb}. V središču omrežja je glavna knjiga (ang. ledger), kjer so zabeležene vse transakcije, izvedene na omrežju. \cite{hyperledgerDocs}
Vsak blok v verigi predstavlja zbirko transakcij, bloke pa med seboj povezuje zgoščevalna (hash) funkcija.
V verigo je blok moč le dodajati.
Vsak nov blok mora biti pred zapisom potrjen s strani omrežja in, preko vrednosti zgoščene funkcije prejšnjega člena, povezan v verigo blokov.
Celotna veriga blokov je shranjena pri vsaki izemd sodelujočih entiteti.
...
Izvajanje storitev se danes seli v oblak, kjer se jih poljubno mnogokrat replicira glede na trenutne potrebe.

Izvajanje storitev želimo decentralizirati - vsaka sodelujoča entiteta v omrežju lahko, pod doloženimi pogoji, izvaja
katerokoli izmed nabora razpoložljivih storitev.
Velika prednost, ki jo prinaša decentralizirano izvajanje poslovne logike je praktično nemogoč napad zavrnitev storitve (DOS) in celo 
porazdeljena zavrnitev storitve (DDOS).
Napadalec je zmožen posamezno sodelujočo entiteto v omrežju obremeniti do te mere, da preneha z izvajanjem določene storitve.
V tem primeru se izvajanje storitve zaupa drugi entiteti, postopek, ki je za uporabnika povsem transparenten.

Vsak klic storitve je možno opremiti s finančnimi podatki, uspešen klic pomeni samodejno izvedeno finančno transakcijo.

\section{Odkrivanje storitev v decentraliziranem okolju}

\subsection{Registracija nove storitve}

Sodelujoče entitete so ob registraciji nove storitve o tem obveščene.
Vsak izmed izvajalcev lahko, glede na uporabniško določene pogoje uporabljenih sistemskih sredstev, prične z izvajanjem nove storitve, če za to obstaja še neizpolnjeno naročilo.


\subsection{Registracija izvajalcev}
Vsaka digitalna entiteta si lahko lasti več izvajalcev storitev.
Način več izvajalcev za posamezno identiteto omogoča izvajanje repliciranih procesov, ki operirajo le z enim uporabniškim računom.
Posameznega izvajalca je potrebno registrirati v omrežju, s čimer je možno prisluženo vrednost posameznega izvajalca pripisati lastniku.
Podatki, ki se o posameznem izvajalcu zabeležijo so: lastnik (account), unikatna številka izvajalca (id) in naslov, na katerem je izvajalec dosegljiv preko omrežja.
Posameznemu izvajalcu se lokalno nastavi tudi omejitve glede porabe sistemskih virov, ki jih lastnik nameni izvajanju storitev.
...
Komunikacija z nadzornim procesom trenutno poteka preko REST zahtevkov, v načrtu pa je podpora za konzolni način upravljanja ter
podpora novejšim komunikacijsim protokolom, kot so gRPC, Apache Trift in podobni.

Za registracijo nove storitve nadzornemu procesu podamo pot do slike vsebnika Docker.
Sistem poskrbi za distribucijo slike v omrežje IPFS in storitev registrira.
Storitev je ob registraciji dosegljiva vsem sodelujočim, ki so o pojavu nove storitve tudi obveščeni preko sistema 
dogodkov.

\section{dApi}
Implementacijo decentraliziranih APIjev (dAPI) želimo poenostaviti in pripraviti rešitev, ki bo za uporabnika transparentna.
Standardno KumuluzEE aplikacijo se z razširitvijo \textit{kumuluz-dapi} enostavno opremi z dodatnimi anotacijami in dopolnitvami
v konfiguracijski datoteki, s tem pa je storitev pripravljena na registracijo v omrežju.
Ob zagonu storitve se le-ta registrira v omrežje.
V glavno knjigo zabeleži, katera storitev se izvaja, kdo jo izvaja, na katerem naslovu je storitev dosegljiva ter način oziroma tip komunikacije.

Odkrivanje storitev poteka preko branja trenutnega stanja glavne knjige, iz katere pridobimo naslov, kjer se željena storitev izvaja.
